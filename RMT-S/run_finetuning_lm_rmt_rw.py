...
if __name__ == '__main__':
    ...
    if args.use_adapter:
        ...
        from rmt_rw import GPT2LMHeadModel
        if not args.from_pretrained:
            model_cfg = AutoConfig.from_pretrained(args.model_cfg)
            model = GPT2LMHeadModel(model_cfg)
        else:
            logger.info(f'Loading pretrained model: {args.from_pretrained}')
            model = GPT2LMHeadModel.from_pretrained(args.from_pretrained, use_safetensors=False)
    ...
    if args.num_mem_tokens is not None:
        ...
        for i in range(12):
            # Initialize c_attn weights and biases with Xavier
            nn.init.xavier_uniform_(model.transformer.h[i].attn_rw.c_attn.weight.data)
            nn.init.constant_(model.transformer.h[i].attn_rw.c_attn.bias.data, 0)
            
            # Initialize c_proj weights and biases with Xavier
            nn.init.xavier_uniform_(model.transformer.h[i].attn_rw.c_proj.weight.data)
            nn.init.constant_(model.transformer.h[i].attn_rw.c_proj.bias.data, 0)

        # for i in range(12):
        #     model.transformer.h[i].attn_rw.c_attn.weight.data = base_model.h[i].attn.c_attn.weight.data.clone()
        #     model.transformer.h[i].attn_rw.c_attn.bias.data = base_model.h[i].attn.c_attn.bias.data.clone()
        #     model.transformer.h[i].attn_rw.c_proj.weight.data = base_model.h[i].attn.c_proj.weight.data.clone()
        #     model.transformer.h[i].attn_rw.c_proj.bias.data = base_model.h[i].attn.c_proj.bias.data.clone()
