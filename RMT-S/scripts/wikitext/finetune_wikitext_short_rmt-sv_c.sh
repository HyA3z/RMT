...
accelerate launch --num_processes $NP --config_file ./accelerate.yaml run_finetuning_lm_rmt_c.py \
        --task_name $TASK_NAME \
        --model_path ../draft/rmt_c_4_f_/iter200k/${TASK_NAME}/$MODEL_NAME/${SCHEDULER}_adamw_wd${LR}_${INPUT_SEQ_LEN}-${TGT_LEN}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_from_cpt_0-1/run_$N \
