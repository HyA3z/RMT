...
if __name__ == '__main__':
    ...
    # Newly add
    def filter_by_length(example, input_seq_len):
        return len(example['input_ids']) == args.input_seq_len
    ...
    with accelerator.main_process_first():
        train_dataset = tokenized_datasets["train"].map(lambda x: group_texts(x, block_size, history_size),
                                                        batched=True, desc=f"Grouping train in chunks of {block_size} and history {history_size}").filter(lambda x: filter_by_length(x, args.input_seq_len))
        valid_dataset = tokenized_datasets["validation"].map(lambda x: group_texts(x, block_size, history_size), 
                                                             batched=True, desc=f"Grouping valid in chunks of {block_size}").filter(lambda x: filter_by_length(x, args.input_seq_len))
    ...
    if 'test' in tokenized_datasets.keys():
        test_dataset = tokenized_datasets["test"].map(lambda x: group_texts(x, block_size, history_size),
                                                      batched=True, desc=f"Grouping test in chunks of {block_size}").filter(lambda x: filter_by_length(x, args.input_seq_len))
    ...
    if args.num_mem_tokens is not None:
        ...
        def initialize_weights(module):
            if isinstance(module, (torch.nn.Linear, torch.nn.Embedding)):
                module.weight.data.normal_(mean=0.01, std=config.initializer_range)
        ...
        compress_module.apply(initialize_weights)
