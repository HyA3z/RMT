...
class RMTModel(nn.Module):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([RMTBlock(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class RMTLMHeadModel(nn.Module):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__()
        self.transformer = RMTModel(config)
